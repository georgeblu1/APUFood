...
public class UpdateFood extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        char foodtype =  request.getParameter("foodtype").charAt(0);
        String fooddesc =request.getParameter("fooddesc");
        
        try (PrintWriter out = response.getWriter()) {
            s.setId(id);
            s.setFoodname(foodname);
            s.setPrice(foodprice);
            s.setDescription(fooddesc);
            s.setQuantity(foodquantity);
            s.setFoodtype(foodtype);
            foodDetailsFacade.edit(s);
            
            out.println("Your food details has been updated<br><br>");
            request.getRequestDispatcher("ViewFood").forward(request, response);
