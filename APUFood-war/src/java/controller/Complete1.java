 ...
 * @author George
 */
@WebServlet(name = "Complete1", urlPatterns = {"/Complete1"})
public class Complete1 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Long tempid = Long.parseLong(request.getParameter("a"));
        String rating = request.getParameter("b");

        TempFoodOrder x = tempFoodOrderFacade.find(tempid);

        StaffDetails y = staffDetailsFacade.find(a.getId());
        try (PrintWriter out = response.getWriter()) {
            
        if (x != null) {
            x.setStatus("Complete");
            x.setRating(rating);
            y.setStaffstatus("Available");
            tempFoodOrderFacade.edit(x);
